...
def broken(stringar:list, ifer:str) -> str:
  retval = ""
  for i in range(1,5):
    retval = retval + 'IF(VALUE(' + ifer + ')<' + str(i*25) + ',' + choose(stringar[((i-1)*50):(i*50)], ifer='VALUE('+ifer+')-'+str((i-1)*25)) + (',' if i<4 else ')')
  return retval + ')))'
